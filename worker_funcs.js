...
postMessage({
cmd:"setUI",key:k+"_progress",properties:r
});}
...
postMessage({
cmd:"render",x:x,y:y,sx:sx,sy:sy,dx:dx,dy:dy,rgb24:rgb24,vram:vram_arr
},[vram_arr.buffer]);
};
var pSound_arrs=[];
...
postMessage({
cmd:"SoundFeedStreamData",pSound:pSound_arr,lBytes:lBytes
},[pSound_arr.buffer]);
};
...
value:e.loaded,max:e.total,hidden:false
});
}else cout_print(e.loaded+"bytes");
document.getElementById("start").disabled=false;
};
reader.onload=function(e){
cout_print(""+iso_name+" loaded");
set_progress('readfile',{
value:1,max:1,hidden:false
});
...
default:
postMessage({
cmd:"print",txt:"unknown command "+data.cmd
});}};
